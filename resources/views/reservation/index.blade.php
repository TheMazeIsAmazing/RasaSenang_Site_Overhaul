            ...
            @if (Auth::user()->role == 0)
                <form method="post"
                    ...
                    <div class="form-group mb-3 flex">
                        <label for="date_or_other" style="display:none;"></label><select id="date_or_other"
                                                                                         name="date_or_other"
                                                                                         class="form-control">
                            <option value="time" @if(session('value_date_or_other') == 'time')selected @endif>Datum
                            </option>
                            <option value="other" @if(session('value_date_or_other') == 'other')selected @endif>Nummer,
                                E-mail, Telefoonnummer en meer
                            </option>
                        </select>
                        @error('time_or_other')
                    ...
                    <div id="hidden_if_date_is_selected" class="form-group mb-3">
                        <label for="other" style="display:none;"></label><input id="other" type="text" name="other"
                                                                                placeholder="Typ hier"
                                                                                class="form-control"
                                                                                @if(session('value_other'))value="{{session('value_other')}}" @endif>
                        @error('other')
                        <span class="alert-danger"> {{$message}}</span>
                        @enderror
                    </div>
                    <div id="hidden_if_other_is_selected" class="form-group mb-3">
                        <label for="date" style="display:none;"></label><input id="date" type="date" name="date"
                                                                               class="form-control"
